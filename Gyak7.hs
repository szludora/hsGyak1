module Gyak7 where
    ...
    sumFruits [] = 0
    sumFruits (x:xs) = numberOfFruits x + sumFruits xs where
        numberOfFruits (FruitBatch _ x) = x 
    ...
    sumDifferentFruits batches = helper batches (0,0,0) where
        helper [] (apples, grapes, pears) = (apples, grapes, pears)
        helper (FruitBatch Apple x:bs) (apples, grapes, pears)  = helper bs (apples + x, grapes, pears)
        helper (FruitBatch Grape x:bs) (apples, grapes, pears) = helper bs (apples, grapes + x, pears)
        helper (FruitBatch Pear x:bs) (apples, grapes, pears) = helper bs (apples, grapes, pears + x)
    ...
    instance Num NumberOfApples where
        ...
        fromInteger a = NumberOfApples a
